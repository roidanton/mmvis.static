...
Die Arbeiten von Edelsbrunner et al \cite{edelsbrunner2004timeVaryingReebGraphs} und Szymczak \cite{szymczak2005contourEvolutionInTimeDependentScalarFields} beinhalten topologische Analysen, um Zeitdaten mit Hilfe der Evolution von zeitveränderlichen Reebgraphen bzw. \contours{Konturbäumen}{Konturbaum} zu analysieren. Sie nutzen ebenfalls kontinuierliche Daten bzw. ein zeitabhängiges Skalarfeld über einem Gitter, so dass hier dieselben Einschränkungen für die Übertragung auf den vorliegenden Partikeldatensatz, wie bei den bereits erwähnten Arbeiten über die Skelettextraktion und \contours{Konturbäume}{Konturbaum}, gelten.
...
Überraschenderweise haben zwei der Experten die Ereignisse unabhängig der Strukturen eingeordnet (Antwort drei), was im Sinne der Ereignisvisualisierung falsch ist. Dies deutet Schwierigkeiten bei der Interpretation der Visualisierung, die ohne weitere Erklärungen erfolgt, an. Das mit 34\% bedeutend mehr Laien diese Antwort gewählt haben und die Antworten wesentlich gleichverteilter als bei der Expertengruppe sind, weist auf einen Rateprozess seitens dieser Gruppe bei der Antwortermittlung hin. Dies bedeutet, dass die Beurteilung ohne Hintergrundwissen schwer fällt. Auch bezog sich ein Großteil der Rückfragen zur Umfrage seitens der Laien auf die Frage neun.
	...
	\ffigbox[\FBwidth] {
		\begin{subfloatrow}
			\vbox {%
				...
				{\caption{\ereignis{Birth} und \ereignis{Death} und die zugehörigen \CFDterms{Einpartikelcluster}{Nulltiefenwert-Einpartikelcluster} mit einer Partikelgröße von $0,15$ in der \viss{Mikrosicht}{Sicht}. Die Glyphgröße beträgt $\glssymbol{rglyph} = 0,002$. Es werden nur  der Anzeige des aktuellen Zeitpunktes $\glssymbol{rzeitMod} = \text{Current}$ mit einer Spanne $\glssymbol{rzeitSpan} = 0$ sowie der Texturkodierung für die Zeit. $t_S = 101$.}\label{fig:eva:birth-death}}\vss
			}
		\end{subfloatrow}
	}
...
\ereignis{Birth}- und \ereigniss{Deathereignisse}{Death} treten sehr gehäuft an kleinen Clustern auf. Überraschenderweise bilden sie bei der Visualisierung über den gesamten Zeitverlauf mehrere, wie in \autoref{fig:eva:birth-death-flow} abgebildete Ketten und werden daher als \SECterm{Birth-Death-Ketten} bezeichnet. Diese Verläufe sind bei allen überprüften Einstellungen für $\glssymbol{cradMod} = \{5,7,10\}$ analog, nur die Anzahl der Ereignisse nimmt mit steigendem \glssymbol{cradMod} ab (wie in \autoref{sec:eva:ereignis-param} gezeigt).
	...
	\ffigbox[\FBwidth] {
		\begin{subfloatrow}
			\vbox {%
				...
				{\caption{Alle vier \CFDterm{Nulltiefenwert-Einpartikelcluster} besitzen voneinander disjunkte Farben. Somit bilden sie unterschiedliche Cluster. Die Partikel- und Glyphgröße ist identisch zu \autoref{fig:eva:birth-death}. Der Mangel }\label{fig:eva:birth-death-flow:beweis:farbe}}
				\ffigbox[\FBwidth][\FBheight]
				...
				{\caption{Vergößerung der Partikelgröße auf $1$ und der Glyphgröße auf $\glssymbol{rglyph} = 0,004$, so dass die Nähe der Partikel zueinander sichtbar wird.}\label{fig:eva:birth-death-flow:beweis:size}}\vss
			}
		\end{subfloatrow}
	}
...
\begin{figure}
	...
	\caption{Diese Aufnahme weist die identische Position, dieselbe Zeit $\glssymbol{ts} = 101$, die gleiche Clusterfärbung ($\glssymbol{cfarbMod} = \text{AllRd}$) und die übereinstimmenden Ereignisdaten von \autoref{fig:eva:birth-death} auf. Jedoch werden durch die Einstellung $\glssymbol{rzeitMod} = \text{All}$ die Glyphen aller Zeitpunkte dargestellt, wodurch eine \SECterm{Birth-Death-Kette} aufgezeigt werden kann.}\label{fig:eva:bdkette}
\end{figure}
	...
	\ffigbox[\FBwidth] {
		\begin{subfloatrow}
			\vbox {%
				...
				{\caption{Das \ereigniss{Splitereignis}{Split} (das Rechte der beiden \ereignis{Split}ereignisse) wird erkannt bei einem Anteil von 40\% gemeinsamer Partikel $\glssymbol{emsA} = 40$. Das \ereignis{Merge}ereignis (rechts) kann als \SECterm{Ereignisrauschen} betrachtet werden.}\label{fig:eva:spliterkennung-ms2-40}}\vss
				\ffigbox[\FBwidth][\FBheight]
				{
...
Die Berechnungen lassen mit Hilfe ihrer sowohl quantitativen als auch qualitativen Ausgabe Erkenntnisse über die Veränderung der Strukturen während der Simulation zu, da die Anzahl der Cluster und die Verteilung der Partikel stark auf Topologieänderungen reagiert. Der Nutzer kann Parameter ändern, um etwa \SECterm{Ereignisrauschen} zu verringern und durch die Möglichkeit, die Berechnungen permanent zu speichern, auch schnell zwischen den Parametern für die einzelnen Berechnungsschritte wechseln. Mit dem Vergleich der Ergebnisse verschiedener Parametereinstellungen, können die individuellen Nachteile der jeweiligen Einstellung ausgeglichen werden; beispielsweise ist die Erfassung aller wesentlichen Ereignisse durch einen kleinen \CFDterm{Partikelradiusmultiplikator} \glssymbol{cradMod} sowie einem niedrigen Schwellwert für die Anteile gemeinsamer Partikel \glssymbol{emsA} möglich. Diese Einstellungen besitzen allerdings den Nachteil, ein starkes \SECterm{Ereignisrauschen} zu erzeugen. Diese Ergebnisse können mit den Ergebnissen einer Berechnung mit restriktiven Einstellungen, das heißt mit einem großen \glssymbol{cradMod} und einem hohen Schwellwert \glssymbol{emsA}, verglichen werden. Die zweite Berechnung erzeugt ein geringes \SECterm{Ereignisrauschen}, erfasst jedoch im Gegenzug einige wesentliche Ereignisse nicht (siehe \autoref{sec:eva:ereignis-param}, \autoref{eva:ereignis-quali-param}).

Trotz dessen, dass die erhoffte Umsetzung der Berechnungen mit \contours{Konturbäumen}{Konturbaum} nicht erfolgen konnte, wurde die Zielstellung, Strukturveränderung mittels \viss{Ereignisglyphen}{Ereignisglyph} abzubilden, durch die Entwicklung eigener Algorithmen in Form des \CFD und der \SECC sowie der darauf aufbauenden \SECterm{Ereignisheuristik} erreicht. Diese geben dem Nutzer Mittel in die Hand, die fünf Schwellwerte \glssymbol{cradMod}, \glssymbol{cminCG}, \glssymbol{emsA}, \glssymbol{emsP} und \glssymbol{ebd} zu setzen, die die Qualität und Anzahl der erkannten Ereignisse stark beeinflussen. Weiterhin ergab eine Umfrage unter Experten und Laien, dass die Bedeutung der Glyphen intuitiv eingeschätzt werden kann. Lediglich die Zuordnung der Helligkeit für die Darstellung der Ereigniszeit sollte vertauscht werden. Auch kann unter Berufung auf die Umfrageergebnisse die Empfehlung erfolgen, die Farbdarstellung für die Ereigniszeit nur in der \viss{Makrosicht}{Sicht} zu nutzen und für die \viss{Mikrosicht}{Sicht} stattdessen den Füllstand oder die Helligkeit zu verwenden. Dies wird über den Parameter \glssymbol{rvisZeit} gesteuert. Daneben helfen \glssymbol{rglyph} beim Wechsel zwischen \viss{Sichten}{Sicht} und \glssymbol{reaMod} bei der gezielten Analyse einer Ereignisart. Die zwei implementierten Parameter zur Veränderung der Zuweisung von \viss{visuellen Variablen}{visuelle Variable} zu Ereignisort \glssymbol{rvisPos} und Ereignisart \glssymbol{rvisArt} dienen für eine Weiterentwicklung des Plugins unter Nutzung anderer Anordnungen von \viss{Ereignis-}{Ereignisobjekt} und \vis{Partikelobjekt} (siehe \autoref{sec:ausblick}).
